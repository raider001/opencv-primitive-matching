...
**Goal:** Produce a single cross-technique HTML report by **collating** the per-technique
`report.html` files already generated by Milestones 7–19, rather than re-running every
matcher.  The benchmark test reads each existing report's data from disk, merges the
per-technique result metadata, and writes a single unified view.

**Key constraint:** `MatchingBenchmarkTest` does **not** re-run any matcher.  It reads the
already-generated per-technique reports and aggregates them.
...
- `src/test/java/org/example/MatchingBenchmarkTest.java`:
  ...
  - `@Test runFullBenchmark()` -- instantiates all matchers (base + CF + CF1 variants),
    runs the double loop (47 refs x 1,267 scenes) for every technique
...
## Milestone 21 -- Multi-Colour-First Region Proposal

**Goal:** Extend the Colour-First Region Proposal (Milestone 15) from single-colour to
multi-colour awareness. The current CF1 engine extracts exactly one HSV window — the
dominant foreground colour of the reference. This milestone adds the ability to extract
**all perceptually distinct foreground colours** present in a reference image, propose one
set of candidate windows **per colour channel**, merge the window sets, and run the
matcher inside the union.

This is motivated by multi-colour reference shapes such as:
- A PCB fiducial with a red outline and a green fill
- A compound shape where different components carry distinct hues
- Any target where masking only one colour causes large portions of the shape to be hidden

### New reference shapes required

Before the engine can be validated, the reference library must include multi-colour shapes
that exercise the new code paths. The following shapes will be added to `ReferenceId` and
`ReferenceImageFactory`:

| New `ReferenceId` | Description |
|---|---|
| `BICOLOUR_CIRCLE_RING` | Circle with a coloured outline (foreground palette colour) and a contrasting filled centre (complementary hue, palette colour + 90° on the colour wheel) |
| `BICOLOUR_RECT_HALVES` | Rectangle split horizontally — top half one hue, bottom half a second hue (≥ 60° apart) |
| `TRICOLOUR_TRIANGLE` | Equilateral triangle with three regions painted in three distinct hues (~120° apart) |
| `BICOLOUR_CROSSHAIR_RING` | Crosshair lines in one colour surrounded by a circle ring in a second colour |
| `BICOLOUR_CHEVRON_FILLED` | Chevron outline in colour A, interior region flood-filled with colour B |

These references are always drawn on a **solid black canvas** to avoid hue contamination
from gradient backgrounds.

### Key design questions and decisions

**1 — Colour extraction: explicit vs histogram-based**

- **Option A (explicit annotation):** `ReferenceImageFactory` exposes
  `List<Scalar> foregroundColours(ReferenceId id)` returning the exact BGR colours used
  when drawing. Deterministic; requires per-shape registration.
- **Option B (histogram cluster extraction):** at runtime compute a hue histogram of the
  reference's non-black foreground pixels, find peaks separated by ≥ `MIN_HUE_SEPARATION`,
  treat each peak as a colour channel. More general; can fail on low-saturation references.

**Decision:** Option A for explicitly coded references (controlled, no false peaks).
Option B available as a fallback overload on `ColourFirstLocator.proposeMulti()` for
ad-hoc references where the exact colours are not registered.

**2 — Minimum hue separation (`MIN_HUE_SEPARATION`)**

Two colours are considered perceptually distinct if their hue distance in OpenCV's
0–179 HSV space exceeds this threshold:

| Value | Effect |
|---|---|
| < 10° | Similar hues (e.g. red H=2 and red-orange H=8) treated as the same → duplicate windows, no coverage benefit |
| 25° (recommended default) | Cleanly separates the 8-palette hues (which use ~45° steps); tolerates minor hue drift |
| > 45° | Risk of incorrectly merging genuinely distinct colours on the palette |

The report should show results at **15°, 25°, and 40°** to illustrate coverage vs. redundancy.

**3 — Saturation floor (`MIN_SATURATION`)**

Colours with S < threshold are near-grey, have unstable hue, and produce noisy masks:
- White (S ≈ 0), light grey (S ≈ 20) → skip
- **Default: `MIN_SATURATION = 40`** (out of 255)
- Prevents single-colour white/grey references (palette slot 0) from generating spurious channels

**4 — Window merging strategy**

Each colour channel produces its own `List<Rect>`. These are merged via the existing
`mergeOverlapping` logic with one additional dedup pass across channels:
- **Union with dedup merge (default):** all windows from all channels included, then any
  overlapping pairs collapsed. Strictly a superset of single-colour CF1; degrades
  gracefully to CF1 behaviour when only one channel passes the saturation floor.
- **Intersection-biased (alternative):** only windows present in ≥ 2 channels retained.
  More precise but risks missing windows where one colour is partially occluded.

**5 — Performance impact**

For a reference with N distinct colours (passing the saturation floor):
- Additional pre-filter cost: ~N × 1 ms per scene at 640×480
- Window count: at most N × single-colour window count (typically less after merge)
- Single-colour references: N = 1 → identical cost and behaviour to Milestone 15

### `ColourFirstLocator` API extensions

```java
// New alongside the existing single-colour API:

/** Proposes windows for a list of explicit BGR colours (one set of windows per colour, merged). */
List<Rect> propose(Mat scene, List<Scalar> bgrColours, double hueTolerance, int minArea)

/** Auto-extracts distinct colours from the reference image and proposes merged windows. */
List<Rect> proposeMulti(Mat scene, ReferenceId refId, double hueTolerance,
                         int minArea, int minHueSep, int minSaturation)

/** Extracts the list of perceptually distinct foreground BGR colours from a reference Mat. */
static List<Scalar> extractDistinctColours(Mat refMat, int minHueSep, int minSaturation)
```

### New variant names

Each of the 9 technique families gains 2 new variants alongside CF, CF1:

| Suffix | Tolerance | Engine |
|---|---|---|
| `_MCF1_LOOSE` | ±15° hue per channel | Multi-colour `proposeMulti()` |
| `_MCF1_TIGHT` | ±8° hue per channel | Multi-colour `proposeMulti()` |

This gives a clear four-way comparison per base method:
**Base → CF (full-scene mask) → CF1 (single-colour windows) → MCF1 (multi-colour windows)**

### New test

`src/test/java/org/example/matchingtests/MultiColourFirstTest.java`:
- Extends `AnalyticalTestBase`
- `REF_FILTER` — the 5 new multi-colour references plus 3 single-colour controls
- `SCENE_VARIANTS` — standard difficulty tiers plus the colour-shift degradation variant
- Runs all 9 matchers; saves MCF1_LOOSE and MCF1_TIGHT annotated PNGs
- Output: `test_output/multi_colour_first/report.html`

### Done when
- `mvn test` produces `test_output/multi_colour_first/report.html`
- `extractDistinctColours` correctly identifies 2 colours for `BICOLOUR_CIRCLE_RING`
  and 3 colours for `TRICOLOUR_TRIANGLE`; 1 colour for a standard single-hue reference
- MCF1 score ≥ CF1 score on multi-colour references in A_CLEAN and B_TRANSFORMED
- MCF1 score ≈ CF1 score on single-colour control references (graceful degradation)
- MCF1 pre-filter adds ≤ N × 1 ms per extra colour at 640×480
- D_NEGATIVE: MCF1 score ≈ 0% (no windows → full-scene fallback returns near-zero)
...
| 20 | Unified Benchmark (collator) | `benchmark/report.html` — collates all per-technique reports; no re-run |
| 21 | Multi-Colour-First Region Proposal | `multi_colour_first/report.html` — MCF1 variants (×9 techniques × 2 tolerances); 5 new multi-colour reference shapes; `extractDistinctColours` API |
