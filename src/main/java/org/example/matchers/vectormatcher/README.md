...
**LINE_SEGMENT AR relaxation (in `VectorSignature.similarity`):** For LINE_SEGMENT × LINE_SEGMENT pairs, the `arMultiplier` is bypassed (set to 1.0). The exact aspect ratio from `minAreaRect` is unreliable for thin shapes across scales — a line that is 8 px tall at 128×128 vs 24 px tall at 640×480 has pixel-level variations that amplify the AR difference (e.g. AR 14 vs 9). Both sides are already confirmed as thin shapes (AR > 4.0), so the structural similarity is captured by circularity and solidity instead.

**LINE_SEGMENT topology-validated seg rescue (in `VectorSignature.similarity`):** When both sides are LINE_SEGMENT with strong topology agreement (topo ≥ 0.80) but SegmentDescriptor is in the partial-failure zone (0.10 ≤ seg < 0.50), segScore is floored at `topoScore × 0.85`. The partial failure occurs because CHAIN_APPROX_SIMPLE produces different segment counts at ref scale (128×128) vs scene scale (640×480), but the cyclic topology (edge-length/angle pairs) is more robust. Safety: requires exact type match, circScore ≥ 0.90, solidityScore ≥ 0.85.

**Moderate-circularity polygon coherence boost (in `VectorSignature.similarity`):** For CLOSED_CONVEX_POLY pairs where both sides have moderate circularity (0.55–0.82), polygon approximation produces wildly different vertex counts at different scales (e.g. 7 vs 11 vertices for the same circle-like shape). This cascades to seg, topo, vertexScore, and angleScore all failing. When circScore ≥ 0.90 and solidityScore ≥ 0.85 confirm the shapes are structurally identical, seg/topo/vtx/angle are floored at conservative values (seg: circScore × 0.60, topo: circScore × 0.55, vtx: 0.75, angle: 0.55). Primary case: circle contours from compound shapes (e.g. COMPOUND_TRIANGLE_IN_CIRCLE) classified as CLOSED_CONVEX_POLY with circularity ~0.72. Does not fire when circularity > 0.82 (handled by the stricter near-circular coherence boost).
